export default function HighlightVideo() {
    return (
        <div className="py-5">
            <div className="flex w-full items-center my-5 justify-between border-b-1 border-grey-500">
                <h2 className="font-headline text-2xl">Highlight ล่าสุด</h2>
                <a className="font-halftime">Highlight ล่าสุด <span className=""> > </span></a>
            </div>
            <div className="grid grid-rows-3 grid-flow-col gap-4">
                <div className="row-span-1 col-span-2 bg-white overflow-hidden">
                    <a className="block">
                        <div className="transform transition-all duration-1000 hover:scale-110">
                            <img src="heroplaylist-image.jpg"></img>
                        </div>
                        <div>
                            <span className="font-halftime text-xs">feature</span>
                            <div className="font-fulltime text-lg">บีจียืนยันปล่อยยืม 'โตติ' ซบสมุทรปราการ</div>
                        </div>
                    </a>
                </div>
                <div className="col-span-12 bg-white overflow-hidden">
                    <a className="block">
                        <div className="transform transition-all duration-1000 hover:scale-110">
                            <img src="thumb-1.jpg"></img>
                        </div>
                        <div>
                            <span className="font-halftime text-xs">feature</span>
                            <div className="font-fulltime text-lg">ราชบุรี เปิดตัวแข้งใหม่ 3 ราย ลุยเลก 2</div>
                        </div>
                    </a>
                </div>
                <div className="row-span-1 col-span-12 bg-white overflow-hidden">
                    <a className="block">
                        <div className="transform transition-all duration-1000 hover:scale-110">
                            <img src="heroplaylist-image.jpg"></img>
                        </div>
                        <div>
                            <span className="font-halftime text-xs">feature</span>
                            <div className="font-fulltime text-lg">บีจียืนยันปล่อยยืม 'โตติ' ซบสมุทรปราการ</div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    )
}